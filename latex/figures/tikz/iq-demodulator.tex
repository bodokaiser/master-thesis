...
\begin{document}
	\begin{tikzpicture}
        ...
        \draw (term) ++(3,0) node[oscillator, label={left:$\cos(\omega_0t)$}](osc){};
        ...
        \draw (mixeru.east) to[short] ++(1,0) to[lowpass, >, l=LP] ++(3,0) to[short] ++(1,0) node[inputarrow]{} node[right]{$x(t)$};
        \draw (mixerd.east) to[short] ++(1,0) to[lowpass, >, l=LP] ++(3,0) to[short] ++(1,0) node[inputarrow]{} node[right]{$p(t)$};
	\end{tikzpicture}
\end{document}