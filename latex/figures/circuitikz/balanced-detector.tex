...
\begin{document}
    \begin{tikzpicture}
        \draw (0, 0)
            ...
            (bd) to[full photodiode, l=$\text{PD}_+\;\;$] ++(0, 3) node[ocirc, label=$+V_b$](bias plus){}
            (bd) ++(0, -3) node[ocirc, label=$-V_b$, rotate=180](bias minus){} to[full photodiode, l=$\text{PD}_-\;\;$] ++(0, 3)
