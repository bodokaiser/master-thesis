...
\begin{document}
	\begin{tikzpicture}
		\draw (0,0) node[left]{$\Re\left\{\alpha(t)e^{-i\omega_lt}\right\}$} to[short] ++(1,0)
            node[circ](term){};
        ...
        \draw (term) ++(3,0) node[oscillator, label={left:$\sin(\omega_lt)$}](osc){};
        ...
        \draw (mixeru.east) to[short] ++(1,0) to[lowpass, >, l=LP] ++(1,0) to[short] ++(1,0) node[inputarrow]{} node[right]{$p(t)$};
        \draw (mixerd.east) to[short] ++(1,0) to[lowpass, >, l=LP] ++(1,0) to[short] ++(1,0) node[inputarrow]{} node[right]{$x(t)$};
	\end{tikzpicture}
\end{document}
