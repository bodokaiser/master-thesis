...
\begin{document}
	\begin{tikzpicture}[
		...
		\node (div) [twoportsplitshape, circuitikz/t1={\ \SI{90}{\degree}}, circuitikz/t2={\SI{0}{\degree}}] at (cri-|mix1.south) {};
		\node (osc) [oscillator, left=of div] {};
		\node (add) [adder, right=of div] {};
		\coordinate[right=of add] (out);
		
		\draw[-Latex] (cri.north) -- (cri.north|-ups1.west) -- (ups1.west) node[above left] {$x[km]$};
		\draw[-Latex] (ups1) -- (psh1);
		\draw[-Latex] (psh1) -- (dac1);
		\draw[-Latex] (dac1) -- (lwp1);
		\draw[-Latex] (lwp1) -- (mix1) node[midway, above] {$x(t)$};
		\draw[-Latex] (div) -- (mix1);
		\draw[-Latex] (mix1.east) -- (mix1.east-|add.north) node[above] {$x(t)\sin(\omega_ct)$} -- (add.north);
		
		\draw[-Latex] (cri.south) -- (cri.south|-ups2.west) -- (ups2.west) node[below left] {$p[km]$};
		\draw[-Latex] (ups2) -- (psh2);
		\draw[-Latex] (psh2) -- (dac2);
		\draw[-Latex] (dac2) -- (lwp2);
		\draw[-Latex] (lwp2) -- (mix2) node[midway, above] {$p(t)$};
		\draw[-Latex] (div) -- (mix2);
		\draw[-Latex] (mix2.east) -- (mix2.east-|add.south) node[below] {$p(t)\cos(\omega_ct)$} -- (add.south);
		...
		\draw[-Latex] (osc) -- (div);
		\draw[-Latex] (add) -- (out);
		\draw (out) ++(0.5,0) node[above] {$\Re\{\alpha(t)e^{-i\omega_c t}\}$};
	\end{tikzpicture}
\end{document}
