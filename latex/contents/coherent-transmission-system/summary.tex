...
For simplicity, we assumed a perfect noiseless quantum channel without attenuation and perfect synchronization between the transmitter and receiver clocks.\footnote{\Cref{app:synchronization} summarizes some important techniques to compensate for synchronization error of the optical \gls{lo}s and the \gls{adc} clock.}
...
\end{figure}
\Cref{fig:receiver_signal_processing} summarizes a heterodyning receiver's signal-processing in a block diagram.
...
We found the coherent state transformations for the \gls{iqm} and balanced detector, derived in the previous chapter, to exactly implement the electro-optical up- and down-conversion of the transmitter respectively receiver.
\begin{table}[htb]
  \centering
  \begin{tabular}{lccc}
    \toprule
...
\end{table}
\Cref{tab:electro_optical_signal_processing} summarizes the relation between input and output state/signals for the electro-optical components implementing the up- and down-conversion.
