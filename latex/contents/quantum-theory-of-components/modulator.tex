...
The (electro-optical) modulators allow encoding an electrical signal onto an optical carrier, and in that sense, are the interface between the electrical and optical domains.
The domain crossover occurs at the electro-optical phase modulator, which we attempt to describe as a nonlinear-mixing process mediated by the dielectric.
Following, we construct, in the optical domain, a complex amplitude modulator, or \gls{iqm}, using \glspl{mzi} driven by electro-optical phase modulators.
...
While the former classical approach is perfectly sufficient, it does not fit well into our quantum-mechanical framework using interactions and unitary operators.
...
To simplify the discussion, we approximate the electric field (averages) inside the dielectric with free electric field operators at the cost of correctness relating to, e.g., phase-matching.
...
In \Cref{app:macroscopic_interactions}, we discuss nonlinear processes mediated by the electric susceptibility of a nonabsorptive dielectric and find the
...
To find the effect of the \gls{mzm} on an input coherent-state, we study the cumulative effect of the individual optical components.
...
\end{figure}
In practice, however, we implement the \gls{iqm} using three \glspl{mzm} as depicted in \Cref{fig:iqm}.
