...
The \gls{mzm} uses two electro-optical phase modulators to perform amplitude modulation through interference.
We first derive the state transformation of an \gls{mzm} for a fixed, time-independent phase, i.e., an \gls{mzi}, and then argue how the state transformation generalizes to the time-dependent modulation.
\begin{figure}[htb]
	\centering
...
\end{figure}
\Cref{fig:mzi_symmetric} shows a symmetric \gls{mzi} using free-space optics with one signal input; the other input being in the vacuum state.
...
To find the effect of the \gls{mzi} on a coherent input state, we combine the actions of its individual optical components.
Approximating each of the optical components an ideal optical coupler allows us to write the output Fourier amplitudes of the \gls{mzm} as the matrix product
\begin{equation}
...
The matrix transformation, \cref{eq:mzm_matrix2}, is a specific beam splitter transform.
Dumping one of the outputs makes it equivalent to the spectral filter.
