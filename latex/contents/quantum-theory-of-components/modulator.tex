...
In the following, we focus on the linear electro-optic effect, the Pockels effect, which is present in lithium niobate and commonly used in integrated telecommunication modulators.
...
\begin{figure}[htb]
    ...
    \caption{Pockels cell of length $l$ and diameter $d$: The quantum optical input field, $\hat{E}_1(\omega)$, enters the cell from the left. Inside the Pockels cell, an electric quantum field, $\hat{E}_2(\Omega)$, created by applying a voltage to the capacitor plates (black rectangles), couples with the input field via the refractive index tensor $n_{ij}(\omega,\Omega)$ and produces an quantum optical output field, $\hat{E}_1^\prime(\omega+\Omega)$.}\label{fig:pockels_cell}
\end{figure}
Applying a voltage signal $V(\Omega)$ to the capacitor plates creates an electric field inside the cell, perpendicular to the plates, denoted by $\hat{E}_2(\Omega)$.\footnote{The mean expectation value for the electric field operator inside the cell is $\expval{\hat{E}_2(\Omega)}=V(\Omega)/d$.}
An optical input field, $\hat{E}_1(\omega)$, approaches from the left couples to $\hat{E}_1(\omega)$ through the refractive index tensor, $n_{ij}(\omega,\Omega)$, over the length of the Pockels cell, $l$.
The optical output field, $\hat{E}_1^\prime(\omega+\Omega)$, exiting the cell to the right contains frequency components from both the optical input field, $\hat{E}_1(\omega)$, and the electric field inside the cell, $\hat{E}_2(\Omega)$.
