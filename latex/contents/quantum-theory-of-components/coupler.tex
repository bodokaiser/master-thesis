...
Let us consider how coherent input states transform under coupling.
Let 
\begin{align}
	\ket{\vb{\alpha}(t)}
	&=
	...
	\mathbb{C}^{2\times1}
\end{align}
denote the tensor product of two coherent states.
Let us further adopt the matrix notation for the annihilation operators used in Ref.~\cite[p.~206]{Vogel2006} and write
\begin{align}
	\vu{a}(\omega)
	&=
	\begin{pmatrix}
	...
	.
\end{align}
The action of the coupling on a quantum state is equivalent to having the unitary (evolution) operator act on the quantum state.
For the coherent input state, we have
\begin{equation}
	\hat{U}
	\ket{\vb{\alpha}(t)}
	=
	...
	\ket{0}
\end{equation}
where we used the unitarity of the evolution operator to replace the identity, $\mathbb{1}=\hat{U}^\dagger\hat{U}$, and the invariance of the vacuum state.
...
\begin{equation}
	\begin{split}
		...
		\exp\left\{
			...
			\left\{
				\alpha_1(\omega)
				\hat{a}_1^\dagger(\omega,t)
				+
				\alpha_2(\omega)
				e^{+i\omega t}
				\hat{a}_2^\dagger(\omega,t)
				-
				\text{h.c.}
			\right\}
		\right\}
...
Using the inverse of the transform operator, \cref{eq:unitary_operator}, we find
\begin{equation}
	\begin{split}
		\hat{U}(\Lambda,\Phi,\Psi,\Theta)
		\hat{\vb{a}}(\omega)
		\hat{U}(\Lambda,\Phi,\Psi,\Theta)^\dagger
		&=
		\hat{U}(-\Lambda,-\Psi,-\Phi,-\Theta)^\dagger
		\hat{\vb{a}}(\omega)
		\hat{U}(-\Lambda,-\Psi,-\Phi,-\Theta)
		\\
		&=
		U(-\Lambda,-\Psi,-\Phi,-\Theta)
		\hat{\vb{a}}(\omega)
		\\
		&=
		U(\Lambda,\Phi,\Psi,\Theta)^\dagger
		\hat{\vb{a}}(\omega)
		,
	\end{split}
\end{equation}
or in short notation omitting the angle arguments
\begin{equation}
	\hat{U}
	\hat{\vb{a}}(\omega)
	\hat{U}^\dagger
	...
	.
\end{equation}
Inserting the result back into the transformed displacement operator, we find
\begin{equation}
	\begin{split}
		\hat{D}^\prime\left[\vb{\alpha}(t)\right]
		&=
