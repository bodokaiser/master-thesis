...
The Maxwell field is the quantm field whose excitations correspond to photons, the mediators of the electromagnetic force.
...
First, we motivate the Maxwell Lagrangian from which we derive the \glspl{eom} encoding the field dynamics.
...
In the last two sections, we perform a plane-wave expansion of the Maxwell field and observables, followed by the canonical quantization in the Coulomb gauge.
...
The second and third term in \cref{eq:maxwell_lagrangian_proposal} are redundant~\cite{deRham2014},  e.g.,
\begin{equation}
	\begin{split}
		\int\dd[4]{x}
		\left(
	...
	\end{split}
\end{equation}
where we used partial integration with vanishing boundaries twice and commutation of the partial derivatives.
To remove the redundancy, we set the third coefficient to zero, $c_3=0$.
...
\end{equation}
with $\varepsilon^{\mu\nu\rho\sigma}$ being the completely antisymmetric tensor.
...
\Cref{eq:covariant_inhomogeneous_maxwell_equations} summarizes the inhomogeneous Maxwell equations and follows from the variational calculus of the Maxwell Lagrangian, i.e., represents the Maxwell field's \glspl{eom}.
...
As a four-dimensional vector field, we would expect the Maxwell field $A^\mu$ to have four \glspl{dof}, one temporal $A^0$, one longitudinal $A_\parallel$, and two transverse $\vb{A}_\perp$.
At the same time, freely propagating electromagnetic waves have only two \glspl{dof}, the polarization.\footnote{Alternatively, we can take the particle picture and say that the photon has two helicities, $\pm1$.}
...
Gauge symmetry reflects the non-physicality, more precisely, the mathematical redundancy of two of the four \glspl{dof}.
To remove the unphysical \glspl{dof}, we impose a gauge condition, i.e., we choose a specific gauge field $\chi$ and perform a gauge transformation
\begin{equation}
	A_\mu
	\to
	A_\mu^\prime
...
\begin{equation}
	...
	0	
\end{equation}
has the advantage of being manifestly Lorentz covariant at the cost of having the different \glspl{dof} intertwined to be independent of a particular reference frame.
The Coulomb gauge
\begin{equation}
	\partial_i
	A^i
...
In the canonical quantization of gauge theories, like the Maxwell field, we have the additional technical complication of handling the unphysical \glspl{dof}.
...
We now perform the canonical quantization by replacing the dynamical field variables $A_i,-E_i$ with the field operators $\hat{A}_i,-\hat{E}_i$ satisfying the \gls{etcr}~\cite[p.~197]{Greiner2013}
\begin{align}
