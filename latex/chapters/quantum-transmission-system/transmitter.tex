...
We conclude that the the action of a generic \gls{mzm} is
\begin{equation}
	\ket{\alpha}
	\to
...
\begin{equation}
	...
	\ket{\alpha\cos(\Theta/2)e^{i\Lambda/2}}
\end{equation}
which is also valid for time-dependent phases $\Theta$ and $\Lambda$ when we use the Pockels modulator to induce the phase shifts.

\subsection{In-phase and quadrature modulator}

The \gls{mzm} modulates the amplitude, i.e., multiplies the amplitude with a real number in time-domain.
We now want to multiply the amplitude with a complex number equivalent to modulation of both quadratures.
The \gls{iqm} modulates the quadrature components of coherent light.

 which in the time-domain is equivalent to a multiplication with a complex number, i.e.,
\begin{equation}
	\ket{\alpha(t)}
	\to
	\ket{\alpha(t)z(t)}
\end{equation}
wherein $z(t)$ depends on the differential phases of the \gls{mz}.

 by first splitting the signal into two branches, where each branch has an \gls{mzm}, and then recombining the two branches.
...
\begin{equation}
	...
	.
\end{equation}
For the differential mode phase of the \gls{mzm} around $\pi$, $\Theta_1,\Theta_2\approx\pi$, we find
\begin{equation}
	z(\Theta_1,\Theta_2)
	\approx
	\frac{1}{4}
	...
	\right)
\end{equation}
and we effectively multiplied the initial coherent state amplitude $\alpha$ with a variable complex number $z(\Theta_1,\Theta_2)$ depending on the differential-mode phases of the \gls{mzm}s.

Our results can be easily generalized to time-dependent modulation angles by remembering that the time-dependent phase modulation is a convolution in momentum space which is a linear operation.
The actions we used derive the \gls{iqm} are also linear operations in momentum space, thus, the results should be compatible.