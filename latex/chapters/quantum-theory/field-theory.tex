...
\begin{equation}
	...
	.
\end{equation}
The plane-wave expansion, \cref{eq:maxwell_expansion}, looks extremely similar to the quantum mode expansion of the Maxwell field but relies only on classical arguments.\footnote{The similarity is not surprising if we consider $A(t,\vb{x})$ as the expectation value of the Maxwell field operator given a coherent state, $\bra{\alpha}\vu{A}(t,\vb{x})\ket{\alpha}$.}
...
In canonical quantization, we work in the Hamiltonian picture, replacing the conjugate variables with operators that satisfy the equal-time commutation relations.
...
\begin{align}
	...
	.
\end{align}
Inserting the Maxwell and electric field operators into the equal-time commutation relation, we derive the canonical commutation relations
\begin{align}
	...
	.
\end{align}
We have successfully derived the quantum mode decompositions of the Maxwell and electric field operators.
The next section picks up the annihilation and creation operator from a different perspective by constructing the quantum state space.