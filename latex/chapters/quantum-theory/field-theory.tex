...
Without external currents and charges, $j^\mu=0$, the dynamics of the Maxwell field are determined by the \gls{eom}, \cref{eq:covariant_inhomogeneous_maxwell_equations},
\begin{equation}
	0
	=
	\partial_\mu
	...
	.
\end{equation}
Imposing the Coulomb and temporal gauge, we find the transverse Maxwell field $\vb{A}_\perp$ to satisfy the relativistic wave equation
\begin{equation}
	\partial_t^2
	\vb{A}_\perp
	=
...
Inserting the plane-wave expansion of the Maxwell field
\begin{equation}
	\vb{A}(t,\vb{x})
	=
...
\end{equation}
into the relativistic wave equation constraints the momenta to satisfy
\begin{equation}
	0
	=
	p_0^2
	...
	.
\end{equation}
We define the energy $p_0=\omega(\vb{p})$ to be positive and reproduce the relativistic energy-momentum relation
\begin{equation}
	\omega(\vb{p})
	=
	\norm{\vb{p}}
	...
	.
\end{equation}
By constraining the integration domain in the plane-wave expansion, \cref{eq:maxwell_plane_wave_expansion}, to
\begin{equation*}
	\Sigma
	=
	\left\{
	...
	\right\}
\end{equation*}
we ensure that the plane-wave expansion to satisfy the \gls{eom}.
We rewrite the constrained integration domain of the plane-wave expansion using the delta distribution
\begin{equation}
	\begin{split}
		\vb{A}(t,\vb{x})
		&=
	...
	\end{split}
\end{equation}
and use the composition property of the delta distribution to decompose the delta distribution into a positive and negative frequency part, i.e.,
\begin{equation}
	\delta^{(1)}
	\left(
		p_0^2
	...
	.
\end{equation}
Inserting the positive and negative frequency delta distribution into the plane-wave decomposition, we then find
\begin{equation}
	\begin{split}
		\vb{A}(t,\vb{x})
		&=
	...
	\end{split}
\end{equation}
where we evaluated the delta distribution and we used the conjugate symmetry of the Maxwell field in momentum space, $A(-p_0,-\vb{p})=A(p_0,\vb{p})^*$.
We define the Lorentz-invariant measure
\begin{equation}
	\dd{\mu(\vb{p})}
	=
...
% polarization basis and properties
