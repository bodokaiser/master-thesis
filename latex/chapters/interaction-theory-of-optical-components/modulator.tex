...
We refrain from a further discussion which would require specific knowledge of the wave packet's pulse shape and frequency-dependency of the refractive index.
...
\begin{equation}
	...
	e^{-i\omega_1t}
	+
	\iint_{\mathbb{R}^2}\dd{\omega_1}\dd{\omega_2}
	\chi^{(2)}_{ijk}(\omega_1,\omega_2)
	E^j(\omega_1)E^k(\omega_2)
	e^{-i(\omega_1+\omega_2)t}
\end{equation}
and the Fourier component of the macroscopic polarization components turns out to be~\cite[p.~1070]{Mandel1995}
\begin{equation}
	P_i(\omega)
	=
	\int\dd{\omega_1}
	\chi^{(1)}_{ij}(\omega_1)
	E^j(\omega_1)
	\delta^{(1)}(\omega_1-\omega)
	+
	\iint\dd{\omega_1}\dd{\omega_2}
	\chi^{(2)}_{ijk}(\omega_1,\omega_2)
	E^j(\omega_1)E^k(\omega_2)
	\delta^{(1)}(\omega+\omega_1+\omega_2)
\end{equation}
where the delta distribution ensures energy conservation in the process.
The electric susceptibility tensor $\varepsilon_{ij}(\omega,t)$ is usually defined implicitly through the displacement field
\begin{equation}
	D_i(\omega)
	=
	E_i(\omega)
	...
	P_i(\omega)
	=
	\varepsilon_{ij}E^j(\omega)
\end{equation}
where $\star$ indicates a convolution.
Comparing the expansion of the macroscopic polarization, we find the electric susceptibility tensor to be
\begin{equation}
	\varepsilon_{ij}(t)
	=
	\chi^{(1)}_{ij}(t)
	+
	\int_{\mathbb{R}}\dd{t^\prime}
	\chi^{(2)}_{ijk}(t,t-t^\prime)
	E^j(t^\prime)
	.
\end{equation}
For a dielectric, non-magnetic and non-chiral medium, the refractive index tensor is given by~\cite[eq.~16]{Rerat2020}
\begin{equation}
	n_{ij}
	=
	\sqrt{\varepsilon_{ij}}
...
% form of the susceptibility tensor for LiNiob, see Yariv p. 237
