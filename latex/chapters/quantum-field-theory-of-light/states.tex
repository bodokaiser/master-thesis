...
\begin{example}[Massless Gaussian single-particle number state]
	...
	\end{equation}
\end{example}
\begin{theorem}[Energy observable of the single-paricle number state]\label{thm:single_particle_number_state_energy}
...
\end{definition}
\begin{lemma}[Group velocity of a single-particle number state]
...
\begin{definition}[Coherent state]
	...
	\end{equation}
\end{definition}
\begin{theorem}[Coherent state is eigenstate of the annihilation operator]
...
\begin{lemma}[Energy moments of the coherent state]
	\begin{align}
		\expval{\hat{H}}{\alpha}
		&=
		\int\frac{\dd[3]{p}}{(2\pi)^32\omega(\vb{p})}
