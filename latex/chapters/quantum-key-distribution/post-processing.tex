...
In an alternative scheme, known as multidimensional reconciliation~\cite{Leverrier2008}, Alice and Bob combine multiple variables into a $d$-dimensional vector, e.g., $\vb{x}=(x_1,\dots,x_d)$ respectively $\vb{x}^\prime=(x_1^\prime,\dots,x_d^\prime)$ and agree on a set of symbol vectors $\vu{v}_1,\dots,\vu{v}_m\in\mathbb{R}^d$.
...
\begin{figure}[htb]
	...
	\caption{Multidimensional reconciliation in $\mathbb{R}^2$: Alice and Bob encoded their prepared and measured values into a the two-dimensional vectors $\vb{x}$ (red unfilled diamond) respectively $\vb{x}^\prime$ (red unfilled square). Bob estimates the $11$ symbol vector $\vu{v}_4$ (top right dark green circle) to have the least distance to $\vb{x}^\prime$. He calculates a rotation matrix $R$ which rotates $\vb{x}^\prime$ close to $\vu{v}_4$ as $R\vb{x}$ (bright green filled diamond) and shares the rotational parameters with Alice. Alice applies the rotation to her vector $R\vb{x}$ (bright green filled square). The rotated vectors of Alice and Bob $R\vb{x}$ and $R\vb{x}^\prime$ are close to the symbol vector $\vu{v}_4$ completing the reconciliation procedure.}\label{fig:multidimensional_reconciliation}
\end{figure}
